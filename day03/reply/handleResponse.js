...
module.exports = (newData) =>{
    ...
    if (newData.MsgType === 'text'){
        if ( newData.Content === '啦啦啦') {
            option.content = '我是卖报的菲奥娜，你可以输入“菲奥娜”，来召唤无双剑姬，'
        } else if (newData.Content.indexOf('菲奥娜') !== -1){
            option.content = '无双剑姬-菲奥娜·劳伦特参上\n' +
                             '我以荣誉之名前来杀你。即使你毫无荣誉可言，也一样得死'
        } else if(newData.Content === '3'){
            option.content = '<a href="http://a3524ac9.ngrok.io/search">语音识别</a>'
        }
        else {
        ...
        if (newData.Event === 'subscribe'){
           ...
           if ( newData.EventKey) {
               ...
               options.content = '欢迎扫描召唤师峡谷二维码来到召唤师峡谷,准备好了吗，召唤师\n' +
                   '你可以输入“啦啦啦”，有惊喜哦';
           }
        } else if (newData.Event === 'unsubscribe'){
            console.log('他不关注你了 ovo ');
            // 如果不给值， 微信服务器会请求三次
            option.content = '';
